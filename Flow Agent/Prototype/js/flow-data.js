...
const ACCORDION_STEPS = [
  { num: 1, icon: '⚙', title: 'Get Document By ID', source: 'Adobe Sign', hasMapping: true, expandable: true },
  { num: 2, icon: '⚙', title: 'Put Object', source: 'Amazon Simple Storage Service (S3)', expandable: true },
  { num: 3, icon: '⚙', title: 'Get Document By ID', source: 'Adobe Sign', expandable: true },
  { num: 4, icon: '↕', title: 'If (/getObjectOutput/responseHeaders/Content-Length is greater than or equal to 502000)', expandable: false },
  { num: 5, icon: 'fn', title: 'Flow logCustomMessage', expandable: false, indented: true },
  { num: 6, icon: '↕', title: 'Else', expandable: false },
  { num: 7, icon: '↩', title: 'Exit \'Flow service\' signaling \'Failure\' "uploaded not happened"', expandable: false, indented: true },
  { num: 8, icon: '⚙', title: 'Get Agreement Combined Document', source: 'Adobe Sign', expandable: true },
  { num: 9, icon: '⚙', title: 'IO streamToString', expandable: true },
];

// ── Mapping table data (step 1, before update) ──────────────────────────────
const MAPPING_DATA_ORIGINAL = {
...
// ── Mapping table data (step 1, after update) ────────────────────────────────
const MAPPING_DATA_UPDATED = {
  input: { doc: 'GetDocumentByIDOutput', fields: [{ tag: 'obj', name: 'stream', required: true }] },
  putObject: { doc: 'putObjectInput', fields: [
    { tag: 'str', name: 'objectName', required: true },
    { tag: 'obj', name: 'stream', required: true },
...
const FLOW = {
  ...
  step_test_triggered: {
    type: 'agent',
    userBubble: 'Test with sample data',
  ...
  step_mapping_updated: {
    type: 'agent',
    showReasoning: true,
    agentMessage: `I have made the following updates<br><br><strong>Change to step 1:</strong> Get Document By ID from Adobe Sign<br>— encoding → set to <em>"UTF-8"</em> (was: not mapped)<br><br><strong>Changes to step 3:</strong> Get Document By ID from Adobe Sign<br>— documentId → from <span style="color:#0f62fe">pipeline.id</span> (was: from <span style="color:#0f62fe">GetDocumentByIDOutput.docId</span>)<br>— inputStream → from <span style="color:#0f62fe">getAgreementCombinedDocumentOutput.stream</span> (was: from <span style="color:#0f62fe">putObjectOutput.stream</span>)<br><br>You can now test this with sample data`,

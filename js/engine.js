...
function renderAgentMessage(htmlText, showReasoning, extra, isWelcome) {
  const { row, body } = makeAgentBubbleWrapper();
  const { meta, reasoningBlock } = makeAgentMeta(showReasoning);
  body.appendChild(meta);
  if (reasoningBlock) body.appendChild(reasoningBlock);
  const text = document.createElement('div');
  text.className = isWelcome ? 'agent-text agent-text--welcome' : 'agent-text';
...
function buildDocGroup(docDef, isNested) {
  ...
  docDef.fields.forEach(f => {
    ...
    row.innerHTML = `<span class="tag tag-${f.tag}">${f.tag}</span><span class="mf-name">${f.name}${f.required ? '<span class="required-star">*</span>' : ''}</span>`;
